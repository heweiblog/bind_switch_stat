...
def switch_rootca(stdon, delay, dnsId, commandType, commandId, hashAlgorithm, compressionFormat, encryptAlgorithm):
	def ___switch(target):
		global home, rndc, switch, logger

		try: subprocess.check_call(['ln', '-f', '-s', target, 
			switch], cwd = home)
		except subprocess.CalledProcessError:
			logger.error('[%d] create link path error!' % os.getpid())
			return False

		try: subprocess.check_call([rndc, 'reconfig'], 
				cwd = home)
		except subprocess.CalledProcessError:
			logger.error('[%d] rndc reconfig error!' % os.getpid())
			return False

		try: subprocess.check_call([rndc, 'flush'], cwd = home)
		except subprocess.CalledProcessError:
			logger.error('[%d] rndc flush error!' % os.getpid())
			return False

		logger.warn('[%d] root.ca switch to `%s`' % (os.getpid(), target))
		return True

	def __do_command(target):
		ret = ___switch(target)
	...
	global std, local, logger
	logger.warn('[%d] root.ca will switch in %d seconds' % (os.getpid(), delay))
...
def respond2(cmd, dnsId, hashAlgorithm, compressionFormat, encryptAlgorithm):
	...
	_commandId = int(xmlget(ele, 'commandId'))
	_type = int(xmlget(ele, 'type'))
	_domain = xmlget(ele, 'domain')
	_urgency = int(xmlget(ele, 'urgency'))
	...
	def __do_command_1(domain, serve):
		...
		ret = cmdAck(dnsId, result, hashAlgorithm, compressionFormat, encryptAlgorithm)
		if not ret:
			# FIXME!! we should log the exceptions here
			pass
...
def respond13(cmd, dnsId, hashAlgorithm, compressionFormat, encryptAlgorithm):
	ele = lxml.etree.fromstring(cmd)
...
class DRMSService(ServiceBase):
	...
	def dns_command(ctx, dnsId, randVal, pwdHash, command, commandHash, 
		...
		func = {2:respond2,13:respond13}
		if commandType in func:
			return func[commandType](cmd, dnsId, hashAlgorithm, compressionFormat, encryptAlgorithm)
		'''
		if commandType == 2:
			return respond2(cmd, dnsId, hashAlgorithm, compressionFormat, encryptAlgorithm)

		if commandType == 13:
			return respond13(cmd, dnsId, hashAlgorithm, compressionFormat, encryptAlgorithm)
		'''
	...
	def stupid_xmlget(self, d, tag):
		...
		raise KeyError()
		pass
# end of class HelloWorldService
